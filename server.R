...
server<-function(input, output,session) {
  ...
  output$ls_kon <- renderUI({
    ...
    x <- append(x,list(selectInput("lasso_variable",label="Explanatory variable",c("age","reaction_time","accuracy","IQ","depression","anxiety"))))
    return(x)
  })
  ...
  observeEvent(input$lp,{
    updateTabsetPanel(session,"ls_tab","Regression Plots")
  })
